...
@interface JKSatisfactionViewCell()
...
@property (nonatomic,strong)JKSatisView *satisView;
@end
...
-(void)createSubViews {
    ...
    [self.bgImageView addSubview:self.satisView];
    __weak JKSatisfactionViewCell *weakSelf = self;
    self.satisView.reloadData = ^{
        if (weakSelf.reloadCell) {
            weakSelf.reloadCell();
        }
    };
    self.satisView.clickSubBtn = ^{
        if (weakSelf.submitClicked) {
            weakSelf.submitClicked(weakSelf.model);
        }
    };
}
-(JKSatisView *)satisView {
    if (_satisView == nil) {
        _satisView = [[JKSatisView alloc] init];
    }
    return _satisView;
}
-(void)layoutSubviews {
    [super layoutSubviews];
        ...
        self.bgImageView.frame = CGRectMake(16, 16, minWidth, self.frame.size.height -16);
    self.satisView.frame = CGRectMake(0, 0, minWidth, self.frame.size.height -16);
...
-(void)setModel:(JKMessageFrame *)model {
    _model = model;
    self.satisView.model = model;
}
//-(void)setModel:(JKMessageFrame *)model {
//    _model = model;
//    self.satisView.model = model.sa
//}
//-(void)setModel:(JKMessageFrame *)model {
//    _model = model;
