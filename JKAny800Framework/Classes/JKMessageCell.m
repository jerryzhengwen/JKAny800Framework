...
- (void)setMessageFrame:(JKMessageFrame *)messageFrame{
    ...
    if (isRichText) {
        ...
        if (messageFrame.message.customerNumber >0) {
...
-(BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange {
    if (self.messageFrame.message.customerNumber) {//点击的是业务类型
        int index = [textView.text substringWithRange:characterRange].intValue;
        self.sendMsgBlock([NSString stringWithFormat:@"%d",index]);
        return NO;
    }
